...
void PIOS_MPU_SetGyroBandwidth(uint16_t bandwidth)
{
	...
	if (mpu_dev->mpu_type == PIOS_MPU6500 || mpu_dev->mpu_type == PIOS_MPU9250) {
		...
		if (bandwidth <= 5)
			filter = PIOS_ICM2060X_GYRO_LOWPASS_5_HZ;
		else if (bandwidth <= 10)
			filter = PIOS_ICM2060X_GYRO_LOWPASS_10_HZ;
		else if (bandwidth <= 20)
			filter = PIOS_ICM2060X_GYRO_LOWPASS_20_HZ;
		else if (bandwidth <= 92)
			filter = PIOS_ICM2060X_GYRO_LOWPASS_92_HZ;
		else
		...
		/* Unsupported/higher sample rates */
			...
			filter = PIOS_ICM2060X_GYRO_LOWPASS_250_HZ;
		else
			filter = PIOS_ICM2060X_GYRO_LOWPASS_3281_HZ;
#endif
	}
...
void PIOS_MPU_SetAccelBandwidth(uint16_t bandwidth)
{
	...
	if (mpu_dev->mpu_type == PIOS_MPU6500 || mpu_dev->mpu_type == PIOS_MPU9250) {
		...
		if (bandwidth <= 5)
			...
			filter = PIOS_ICM2060X_ACCEL_LOWPASS_10_HZ;
		else if (bandwidth <= 21)
			filter = PIOS_ICM2060X_ACCEL_LOWPASS_21_HZ;
		else if (bandwidth <= 45)
			filter = PIOS_ICM2060X_ACCEL_LOWPASS_45_HZ;
		else if (bandwidth <= 99)
			filter = PIOS_ICM2060X_ACCEL_LOWPASS_99_HZ;
		else if (bandwidth <= 218)
			filter = PIOS_ICM2060X_ACCEL_LOWPASS_218_HZ;
		else
			filter = PIOS_ICM2060X_ACCEL_LOWPASS_420_HZ;
	}
