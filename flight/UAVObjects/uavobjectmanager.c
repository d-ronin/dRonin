...
static int32_t sendEvent(struct UAVOBase * obj, uint16_t instId,
			...
			void *obj_data, int len)
{
	static volatile uint8_t num_pending = 0;

	static volatile struct PendEvent {
		UAVObjEvent msg;
		void *obj_data;
		int len;
	} pending_events[3];
	...
	if (!in_progress) {
		int num_allowed=10;	/* do at most these many in a pass */

		/* While there are events to pump.. */
		while (num_pending && num_allowed) {
			/* Deallocate the top one.. */
			num_pending--; num_allowed--;
