...
static const struct pios_annunc pios_annuncs_cc3d[] = {
	[PIOS_LED_HEARTBEAT] = {
		.pin = {
			.gpio = GPIOB,
			.init = {
...
const struct pios_annunc_cfg pios_annunc_cfg_cc3d = {
	.annunciators     = pios_annuncs_cc3d,
	.num_annunciators = NELEMENTS(pios_annuncs_cc3d),
};

const struct pios_annunc_cfg * PIOS_BOARD_HW_DEFS_GetLedCfg (uint32_t board_revision)
{
	(void)board_revision;
	return &pios_annunc_cfg_cc3d;
}
