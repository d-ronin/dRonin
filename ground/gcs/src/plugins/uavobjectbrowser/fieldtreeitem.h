...
class EnumFieldTreeItem : public FieldTreeItem
{
    Q_OBJECT
public:
    EnumFieldTreeItem(UAVObjectField *field, int index,
                      const QList<QVariant> &data, TreeItem *parent = 0)
            : FieldTreeItem(index, data, parent),
              m_enumOptions(field->getOptions()), m_field(field)
    {
    }
    ...
    QString enumOptions(int index)
    {
        if ((index < 0) || (index >= m_enumOptions.length())) {
            return QString("Invalid Value (") + QString().setNum(index) +
                   QString(")");
        }
    ...
    void apply()
    {
        ...
        if (value == -1) {
            qDebug() << "Warning, UAVO browser field is outside range. This "
                        "should never happen!";
            Q_ASSERT(0);
            return;
        }
    ...
    void update()
    {
        ...
        UAVDataObject *obj =
                qobject_cast<UAVDataObject *>(m_field->getObject());
...
class IntFieldTreeItem : public FieldTreeItem
{
    Q_OBJECT
public:
    IntFieldTreeItem(UAVObjectField *field, int index,
                     const QList<QVariant> &data, TreeItem *parent = 0)
...
class FloatFieldTreeItem : public FieldTreeItem
{
    Q_OBJECT
public:
    FloatFieldTreeItem(UAVObjectField *field, int index,
                       const QList<QVariant> &data, bool scientific = false,
                       TreeItem *parent = 0)
            : FieldTreeItem(index, data, parent), m_field(field),
