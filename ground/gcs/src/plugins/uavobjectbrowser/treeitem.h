...
public:
    ...
    void setDescription(QString d)
    {
        ...
        if (!d.isEmpty()) {
            // insert html tags to make this rich text so Qt will take care of
            // wrapping
    ...
    inline virtual bool getIsPresentOnHardware() const
    {
...
class MetaObjectTreeItem : public ObjectTreeItem
{
    Q_OBJECT
public:
    MetaObjectTreeItem(UAVObject *obj, const QList<QVariant> &data,
                       TreeItem *parent = 0)
            : ObjectTreeItem(data, parent)
    {
        setObject(obj);
    }
    MetaObjectTreeItem(UAVObject *obj, const QVariant &data,
                       TreeItem *parent = 0)
            : ObjectTreeItem(data, parent)
    {
        setObject(obj);
...
public:
    ...
    virtual void apply()
    {
        foreach (TreeItem *child, treeChildren()) {
            MetaObjectTreeItem *metaChild =
                    dynamic_cast<MetaObjectTreeItem *>(child);
            if (!metaChild)
                child->apply();
        }
    }
    ...
    void setObject(UAVObject *obj)
    {
        ...
        if (dobj) {
            ...
            connect(dobj, SIGNAL(presentOnHardwareChanged(UAVDataObject *)),
                    this, SLOT(doRefreshHiddenObjects(UAVDataObject *)));
            doRefreshHiddenObjects(dobj);
        } else {
            foreach (TreeItem *item, treeChildren()) {
                DataObjectTreeItem *inst =
                        dynamic_cast<DataObjectTreeItem *>(item);
                if (!inst)
...
class InstanceTreeItem : public DataObjectTreeItem
{
    Q_OBJECT
public:
    InstanceTreeItem(UAVObject *obj, const QList<QVariant> &data,
                     TreeItem *parent = 0)
        ...
        if (parent()) {
            if (!isPresentOnHardware)
                ...
                foreach (TreeItem *item, parent()->treeChildren()) {
                    InstanceTreeItem *inst =
                            dynamic_cast<InstanceTreeItem *>(item);
