...
protected:
    int m_refreshInterval; // The interval to replot the curve widget. The data
                           // buffer is refresh as the data comes in.
    PlotDimensions m_plotDimensions;

    QString getUavObjectFieldUnits(QString uavObjectName,
                                   QString uavObjectFieldName)
    {
        ...
        UAVDataObject *obj = dynamic_cast<UAVDataObject *>(
                objManager->getObject(uavObjectName));
        if (!obj) {
            qDebug() << "In scope gadget, UAVObject " << uavObjectName
                     << " is missing";
            return "";
        }
        UAVObjectField *field = obj->getField(uavObjectFieldName);
        if (!field) {
            qDebug() << "In scope gadget, in fields loaded from GCS config "
                        "file, field"
                     << uavObjectFieldName << " of UAVObject " << uavObjectName
                     << " is missing";
            return "";
        }
