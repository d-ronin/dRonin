...
{
    m_refreshInterval =
        50; // TODO: This should not be set here. Probably should come from a define somewhere.
    yAxisUnits = "";
    timeHorizon = 60;
...
SpectrogramScopeConfig::SpectrogramScopeConfig(Ui::ScopeGadgetOptionsPage *options_page)
{
    ...
    colorMapType = (ColorMap::ColorMapType)options_page->cmbColorMapSpectrogram
                       ->itemData(options_page->cmbColorMapSpectrogram->currentIndex())
                       .toInt();
...
ScopeConfig *SpectrogramScopeConfig::cloneScope(ScopeConfig *originalScope)
{
    ...
    for (int i = 0; i < plotCurveCount; i++) {
        Plot3dCurveConfiguration *currentPlotCurveConf =
            originalSpectrogramScopeConfig->m_spectrogramSourceConfigs.at(i);
...
/**
 * @brief SpectrogramScopeConfig::replaceSpectrogramDataSource Replaces the list of spectrogram data
 * sources
 * @param spectrogramSourceConfigs
 */
void SpectrogramScopeConfig::replaceSpectrogramDataSource(
...
/**
 * @brief SpectrogramScopeConfig::loadConfiguration loads the plot configuration into the scope
 * gadget widget
...
void SpectrogramScopeConfig::loadConfiguration(ScopeGadgetWidget *scopeGadgetWidget)
{
    ...
    // There should be only one spectrogram per plot //TODO: Upgrade this to handle multiple
    // spectrograms on a single axis
    ...
    SpectrogramData *spectrogramData = new SpectrogramData(
        uavObjectName, uavFieldName, samplingFrequency, windowWidth, timeHorizon);
    ...
    QString waterfallName =
        (spectrogramData->getUavoName()) + "." + (spectrogramData->getUavoFieldName());
    ...
    UAVDataObject *obj =
        dynamic_cast<UAVDataObject *>(objManager->getObject((spectrogramData->getUavoName())));
    if (!obj) {
    ...
    // Get the units
    QString units =
        getUavObjectFieldUnits(spectrogramData->getUavoName(), spectrogramData->getUavoFieldName());
    ...
    if (spectrogramSourceConfigs->yScalePower == 0)
        ...
        waterfallNameScaled = waterfallName + "(x10^"
            + QString::number(spectrogramSourceConfigs->yScalePower) + " " + units + ")";
    ...
    if (((double)windowWidth) * timeHorizon < (double)10000000.0
            * sizeof(spectrogramData->zDataHistory->front())) { // Don't exceed 10MB for memory
        for (uint i = 0; i < windowWidth * timeHorizon; i++) {
            spectrogramData->zDataHistory->append(0);
        }
    } else {
        qDebug() << "For some reason, we're trying to allocate a gigantic spectrogram. This "
                    "probably represents a problem in the configuration file. TimeHorizion: "
                 << timeHorizon << ", windowWidth: " << windowWidth;
        Q_ASSERT(0);
        return;
    }
...
/**
 * @brief SpectrogramScopeConfig::setGuiConfiguration Set the GUI elements based on values from the
 * XML settings file
 * @param options_page
 */
void SpectrogramScopeConfig::setGuiConfiguration(Ui::ScopeGadgetOptionsPage *options_page)
{
    ...
    // Set the plot type
    options_page->cmb3dPlotType->setCurrentIndex(
        options_page->cmb3dPlotType->findData(SPECTROGRAM));
    ...
    options_page->cmbColorMapSpectrogram->setCurrentIndex(
        options_page->cmbColorMapSpectrogram->findData(colorMapType));
    ...
    foreach (Plot3dCurveConfiguration *plot3dData, m_spectrogramSourceConfigs) {
        ...
        int uavoFieldIdx =
            options_page->cmbUavoFieldSpectrogram->findText(plot3dData->uavFieldName);
...
void SpectrogramScopeConfig::configureAxes(ScopeGadgetWidget *scopeGadgetWidget)
{
    ...
    scopeGadgetWidget->setAxisScale(QwtPlot::xBottom, 0,
