...
Scatterplot2dScopeConfig::Scatterplot2dScopeConfig(Ui::ScopeGadgetOptionsPage *options_page)
{
    ...
    scatterplot2dType = (Scatterplot2dType)options_page->cmbXAxisScatterplot2d
                            ->itemData(options_page->cmbXAxisScatterplot2d->currentIndex())
                            .toInt();
...
ScopeConfig *Scatterplot2dScopeConfig::cloneScope(ScopeConfig *originalScope)
{
    ...
    int scatterplotSourceCount =
        originalScatterplot2dScopeConfig->m_scatterplotSourceConfigs.size();

    for (int i = 0; i < scatterplotSourceCount; i++) {
        Plot2dCurveConfiguration *currentScatterplotSourceConf =
            originalScatterplot2dScopeConfig->m_scatterplotSourceConfigs.at(i);
...
/**
 * @brief Scatterplot2dScopeConfig::replaceScatterplotDataSource Replaces the list of scatterplot
 * data sources
 * @param scatterplotSourceConfigs
 */
void Scatterplot2dScopeConfig::replaceScatterplotDataSource(
    QList<Plot2dCurveConfiguration *> scatterplotSourceConfigs)
{
...
/**
 * @brief Scatterplot2dScopeConfig::loadConfiguration loads the plot configuration into the scope
 * gadget widget
...
void Scatterplot2dScopeConfig::loadConfiguration(ScopeGadgetWidget *scopeGadgetWidget)
{
    ...
    foreach (Plot2dCurveConfiguration *plotCurveConfig, m_scatterplotSourceConfigs) {
        ...
        QString curveName =
            (scatterplotData->getUavoName()) + "." + (scatterplotData->getUavoFieldName());
        ...
        UAVDataObject *obj =
            dynamic_cast<UAVDataObject *>(objManager->getObject((scatterplotData->getUavoName())));
        if (!obj) {
        ...
        // Get the units
        QString units = getUavObjectFieldUnits(scatterplotData->getUavoName(),
                                               scatterplotData->getUavoFieldName());
        ...
        if (plotCurveConfig->yScalePower == 0)
            ...
            curveNameScaled = curveName + "(x10^" + QString::number(plotCurveConfig->yScalePower)
                + " " + units + ")";
        ...
        plotCurve->setPen(QPen(QBrush(QColor(color), Qt::SolidPattern), (qreal)1, Qt::SolidLine,
...
/**
 * @brief Scatterplot2dScopeConfig::setGuiConfigurationSet the GUI elements based on values from the
 * XML settings file
 * @param options_page
 */
void Scatterplot2dScopeConfig::setGuiConfiguration(Ui::ScopeGadgetOptionsPage *options_page)
{
    ...
    // Set the plot type
    options_page->cmb2dPlotType->setCurrentIndex(
        options_page->cmb2dPlotType->findData(SCATTERPLOT2D));
