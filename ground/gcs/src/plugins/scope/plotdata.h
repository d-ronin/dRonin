...
public:
    double valueAsDouble(UAVObject *obj, UAVObjectField *field,
                         bool haveSubField, QString uavSubFieldName);
    ...
    virtual void plotNewData(PlotData *, ScopeConfig *,
                             ScopeGadgetWidget *) = 0;
...
public:
     ...
     */
    enum ColorMapType { STANDARD, JET };

    ColorMap(ColorMapType colorMapType = STANDARD) : QwtLinearColorMap()
    {
        switch (colorMapType) {
        case JET:
            createJet();
