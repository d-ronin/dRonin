...
public:
    ...
    // bool addShadowWidget(QWidget *masterWidget, QWidget *shadowWidget, double shadowScale = 1,
    // bool shadowIsLimited = false, bool shadowUseUnits = false);
    bool addShadowWidget(QString object, QString field, QWidget *widget, int index = 0,
                         double scale = 1, bool isLimited = false, bool useUnits = false,
                         QList<int> *defaultReloadGroups = NULL, quint32 instID = 0);
...
signals:
    ...
    // fired when the framework requests that the widgets values be populated, use for custom
    // behaviour
    void populateWidgetsRequested();
    // fired when the framework requests that the widgets values be refreshed, use for custom
    // behaviour
    void refreshWidgetsValuesRequested();
    // fired when the framework requests that the UAVObject values be updated from the widgets
    // value, use for custom behaviour
...
protected:
    ...
    void checkWidgetsLimits(QWidget *widget, UAVObjectField *field, int index, bool hasLimits,
                            bool useUnits, QVariant value, double scale);
    ...
    /**
     * @brief setWidgetProperty Sets a dynamic property on a widget and forces a re-evaluation of
     * it's stylesheet
