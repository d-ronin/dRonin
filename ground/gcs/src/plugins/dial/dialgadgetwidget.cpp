...
DialGadgetWidget::DialGadgetWidget(QWidget *parent)
...
/*!
  ...
  */
void DialGadgetWidget::connectNeedles(QString object1, QString nfield1, QString object2,
                                      QString nfield2, QString object3, QString nfield3)
{
    if (obj1 != NULL)
...
void DialGadgetWidget::updateNeedle2(UAVObject *object2)
{
    ...
    if (field) {
        if (haveSubField2) {
            int indexOfSubField = field->getElementNames().indexOf(
                QRegExp(subfield2, Qt::CaseSensitive, QRegExp::FixedString));
            value = field->getDouble(indexOfSubField);
        } else
            value = field->getDouble();
        if (value != value) {
...
void DialGadgetWidget::updateNeedle3(UAVObject *object3)
{
    ...
    if (field) {
        if (haveSubField3) {
            int indexOfSubField = field->getElementNames().indexOf(
                QRegExp(subfield3, Qt::CaseSensitive, QRegExp::FixedString));
            value = field->getDouble(indexOfSubField);
        } else
            value = field->getDouble();
        if (value != value) {
                                   ...
                                   QString n3, QString n1Move, QString n2Move, QString n3Move)
{
    ...
    if (QFile::exists(dfn) && m_renderer->load(dfn) && m_renderer->isValid()) {
        ...
        if (m_renderer->elementExists(fg)) {
            ...
            m_foreground->setPos(rectB.width() / 2 - rectF.width() / 2,
                                 rectB.height() / 2 - rectF.height() / 2);
        ...
        m_needle1->setPos(rectB.width() / 2 - rectN.width() / 2,
                          rectB.height() / 2 - rectN.height() / 2);
        ...
        if ((n1 != n2) && n2enabled) {
            ...
            m_needle2->setPos(rectB.width() / 2 - rectN.width() / 2,
                              rectB.height() / 2 - rectN.height() / 2);
        ...
        if (n3enabled) {
            ...
            m_needle3->setPos(rectB.width() / 2 - rectN.width() / 2,
                              rectB.height() / 2 - rectN.height() / 2);
        ...
        // of the needles, so we have to reset the needle values too upon dial file loading,
        // otherwise
