...
UAVObjectField *UAVObject::getField(const QString &name)
{
    ...
    qWarning() << "UAVObject::getField Non existant field " << name
...
{
    metadata.flags = ACCESS_READWRITE << UAVOBJ_ACCESS_SHIFT
...
{
    return UAVObject::UpdateMode((metadata.flags >> UAVOBJ_TELEMETRY_UPDATE_MODE_SHIFT)
 ...
 */
void UAVObject::SetFlightTelemetryUpdateMode(UAVObject::Metadata &metadata,
...
{
    return UAVObject::UpdateMode((metadata.flags >> UAVOBJ_GCS_TELEMETRY_UPDATE_MODE_SHIFT)
                                 & UAVOBJ_UPDATE_MODE_MASK);
}
