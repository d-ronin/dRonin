...
void SoundNotifyPlugin::extensionsInitialized()
{
    ...
    connect(pm, &ExtensionSystem::PluginManager::objectAdded, this,
            &SoundNotifyPlugin::onTelemetryManagerAdded);
...
void SoundNotifyPlugin::onTelemetryManagerAdded(QObject *obj)
{
    ...
    if (telMngr)
        connect(telMngr, &TelemetryManager::disconnected, this,
                &SoundNotifyPlugin::onAutopilotDisconnect);
}
...
void SoundNotifyPlugin::resetNotification(void)
{
    ...
    foreach (NotificationItem *ntf, _notificationList) {
        ntf->disposeTimer();
        disconnect(ntf->getTimer(), &QTimer::timeout, this,
                   &SoundNotifyPlugin::on_timerRepeated_Notification);
        ntf->disposeExpireTimer();
        disconnect(ntf->getExpireTimer(), &QTimer::timeout, this,
                   &SoundNotifyPlugin::on_timerRepeated_Notification);
    }
}
...
void SoundNotifyPlugin::connectNotifications()
{
    foreach (UAVDataObject *obj, lstNotifiedUAVObjects) {
        if (obj != NULL)
            disconnect(obj, &UAVObject::objectUpdated, this,
                       &SoundNotifyPlugin::on_arrived_Notification);
    }
    if (phonon.mo != NULL) {
        delete phonon.mo;
        phonon.mo = NULL;
    ...
    foreach (NotificationItem *notify, _notificationList) {
        ...
        UAVDataObject *obj =
            dynamic_cast<UAVDataObject *>(objManager->getObject(notify->getDataObject()));
        if (obj != NULL) {
            if (!lstNotifiedUAVObjects.contains(obj)) {
                lstNotifiedUAVObjects.append(obj);

                connect(obj, &UAVObject::objectUpdated, this,
                        &SoundNotifyPlugin::on_arrived_Notification, Qt::QueuedConnection);
            }
        } else {
    ...
    connect(phonon.mo, &QMediaPlayer::stateChanged, this, &SoundNotifyPlugin::stateChanged);
}
...
void SoundNotifyPlugin::on_arrived_Notification(UAVObject *object)
{
    foreach (NotificationItem *ntf, _notificationList) {
        ...
        if (ntf->retryValue() != NotificationItem::repeatInstantly
            && ntf->retryValue() != NotificationItem::repeatOncePerUpdate
    ...
    connect(object, &UAVObject::objectUpdated, this, &SoundNotifyPlugin::on_arrived_Notification,
            Qt::UniqueConnection);
}
...
void SoundNotifyPlugin::on_timerRepeated_Notification()
{
    ...
    qNotifyDebug() << QString("repeatTimer: %1% | %2 | %3")
                          .arg(notification->getDataObject())
...
void SoundNotifyPlugin::on_expiredTimer_Notification()
{
    ...
    if (!_pendingNotifications.isEmpty()) {
        qNotifyDebug() << QString("expireTimer: %1% | %2 | %3")
                              .arg(notification->getDataObject())
                              .arg(notification->getObjectField())
                              .arg(notification->toString());
...
{
    if (notification->getDataObject() != object->getName()
        || object->getField(notification->getObjectField()) == NULL)
        return;
    ...
    if (UAVObjectField::ENUM == field->getType()) {
        qNotifyDebug() << "Check range ENUM" << value.toString() << "|"
                       << notification->singleValue().toString() << "|" << field->getOptions()
                       << "|" << direction
                       << checkRange(value.toString(), notification->singleValue().toString(),
                                     field->getOptions(), direction);
        ;
        condition = checkRange(value.toString(), notification->singleValue().toString(),
                               field->getOptions(), direction);
    } else {
        qNotifyDebug() << "Check range VAL" << value.toString() << "|"
                       << notification->singleValue().toString() << "|" << field->getOptions()
                       << "|" << direction
                       << checkRange(value.toDouble(), notification->singleValue().toDouble(),
                                     notification->valueRange2(), direction);
        condition = checkRange(value.toDouble(), notification->singleValue().toDouble(),
                               notification->valueRange2(), direction);
    }
    ...
    if (notification->retryValue() == NotificationItem::repeatOncePerUpdate
        && notification->getCurrentUpdatePlayed())
        return;
    ...
    if (!playNotification(notification)) {
        if (!_pendingNotifications.contains(notification)
            ...
            // QxtTimer::singleShot(notification->getExpireTimeout()*1000, this,
            // SLOT(expirationTimerHandler(NotificationItem*)), qVariantFromValue(notification));
            _pendingNotifications.append(notification);
            notification->startExpireTimer();
            connect(notification->getExpireTimer(), &QTimer::timeout, this,
                    &SoundNotifyPlugin::on_expiredTimer_Notification, Qt::UniqueConnection);
        }
    }
}
...
bool SoundNotifyPlugin::playNotification(NotificationItem *notification)
{
    ...
    if ((phonon.mo->state() == QMediaPlayer::PausedState)
        || (phonon.mo->state() == QMediaPlayer::StoppedState) || phonon.firstPlay) {
        _nowPlayingNotification = notification;
        notification->stopExpireTimer();

        if (notification->retryValue() == NotificationItem::repeatOnce) {
            _toRemoveNotifications.append(
                _notificationList.takeAt(_notificationList.indexOf(notification)));
            ...
            if (notification->retryValue() != NotificationItem::repeatInstantly) {
                ...
                int timer_value = 0;
                int pos =
                    rxlen.indexIn(NotificationItem::retryValues.at(notification->retryValue()));
                if (pos > -1) {
                ...
                connect(notification->getTimer(), &QTimer::timeout, this,
                        &SoundNotifyPlugin::on_timerRepeated_Notification, Qt::UniqueConnection);
            }
        }
        phonon.mo->stop();
        ...
        phonon.firstPlay =
            false; // On Linux, you sometimes have to nudge Phonon to play 1 time before
