    ...
    , _mute(false)
{
    ...
    NotificationItem::retryValues.insert(repeatOncePerUpdate,
...
{
    settings->setValue("SoundCollectionPath",
                       Utils::PathUtils().RemoveDataPath(getSoundCollectionPath()));
...
void NotificationItem::restoreState(QSettings *settings)
{
    ...
    setSoundCollectionPath(Utils::PathUtils().InsertDataPath(
        settings->value(QLatin1String("SoundCollectionPath"), tr("")).toString()));
...
QString NotificationItem::checkSoundExists(QString fileName)
{
    ...
    if (QFile::exists(filePath))
        ...
        filePath = QDir::toNativeSeparators(getSoundCollectionPath() + "/default/" + name);
        if (!QFile::exists(filePath))
            filePath.clear();
    }
    return filePath;
...
{
    return dynamic_cast<UAVDataObject *>(
        (ExtensionSystem::PluginManager::instance()->getObject<UAVObjectManager>())
