...
public:
    ...
    */
    ...
    /**
    * Returns sound caption name, needed to create string representation of
    * notification.
    *
    * @return success - string  == <sound filename>, if sound file exists
    *         error   - string  == [missind]<sound filename>, if sound file
    * doesn't exist
    */
...
private:
    ...
    //! time from putting notification in queue till moment when notification
    //! became out-of-date
    //! NOTE: each notification has it lifetime, this time setups individually
    ...
    //! one-side range, value(numeric or ENUM type) maybe lower, greater or in
    //! range
