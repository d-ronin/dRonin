...
QWidget *NotifyPluginOptionsPage::createPage(QWidget * /* parent */)
{
    ...
    _optionsPage->SoundDirectoryPathChooser->setPromptDialogTitle(
        tr("Choose sound collection directory"));

    connect(_optionsPage->SoundDirectoryPathChooser, &Utils::PathChooser::changed, this,
            &NotifyPluginOptionsPage::on_clicked_buttonSoundFolder);
    ...
    connect(this, &NotifyPluginOptionsPage::updateNotifications, _owner,
            &SoundNotifyPlugin::updateNotificationList);
...
void NotifyPluginOptionsPage::finish()
{
    disconnect(_optionsPage->UAVObjectField,
               ...
               &NotifyPluginOptionsPage::on_changedIndex_UAVField);

    disconnect(_testSound, &QMediaPlayer::stateChanged, this,
               &NotifyPluginOptionsPage::on_changed_playButtonText);
    if (_testSound) {
        _testSound->stop();
    }
}
...
void NotifyPluginOptionsPage::initButtons()
{
    ...
    connect(_optionsPage->buttonAdd, &QAbstractButton::pressed, this,
            &NotifyPluginOptionsPage::on_clicked_buttonAddNotification);
    connect(_optionsPage->buttonDelete, &QAbstractButton::pressed, this,
            &NotifyPluginOptionsPage::on_clicked_buttonDeleteNotification);
    connect(_optionsPage->buttonModify, &QAbstractButton::pressed, this,
            &NotifyPluginOptionsPage::on_clicked_buttonModifyNotification);
    connect(_optionsPage->buttonPlayNotification, &QAbstractButton::clicked, this,
            &NotifyPluginOptionsPage::on_clicked_buttonTestSoundNotification);
}

void NotifyPluginOptionsPage::initPhononPlayer()
{
    _testSound = new QMediaPlayer;
    connect(_testSound, &QMediaPlayer::stateChanged, this,
            &NotifyPluginOptionsPage::on_changed_playButtonText);
}
...
void NotifyPluginOptionsPage::initRulesTable()
{
    ...
    connect(_notifyRulesSelection, &QItemSelectionModel::selectionChanged, this,
            &NotifyPluginOptionsPage::on_changedSelection_notifyTable);
    connect(this, &NotifyPluginOptionsPage::entryUpdated, _notifyRulesModel.data(),
            &NotifyTableModel::entryUpdated);
...
{
    return (_currUAVObject) ? _currUAVObject->getField(_selectedNotification->getObjectField())
                            : NULL;
}
...
void NotifyPluginOptionsPage::setSelectedNotification(NotificationItem *ntf)
{
    ...
    _currUAVObject = dynamic_cast<UAVDataObject *>(
        _objManager.getObject(_selectedNotification->getDataObject()));
    if (!_currUAVObject) {
...
void NotifyPluginOptionsPage::addDynamicField(UAVObjectField *objField)
{
    ...
    disconnect(_dynamicFieldCondition,
               QOverload<const QString &>::of(&QComboBox::currentIndexChanged), this,
               &NotifyPluginOptionsPage::on_changedIndex_rangeValue);
    ...
    if (cond < 0)
        return;
    _dynamicFieldCondition->setCurrentIndex(
        _dynamicFieldCondition->findText(NotifyPluginOptionsPage::conditionValues.at(cond)));

    connect(_dynamicFieldCondition, QOverload<const QString &>::of(&QComboBox::currentIndexChanged),
...
void NotifyPluginOptionsPage::addDynamicFieldWidget(UAVObjectField *objField)
{
    ...
    default:
        ...
        if (NotifyPluginOptionsPage::conditionValues.indexOf(_dynamicFieldCondition->currentText())
            == NotifyPluginOptionsPage::inrange) {
        ...
        if (QComboBox *cbValue = dynamic_cast<QComboBox *>(_dynamicFieldWidget)) {
            ...
            if (QLineEdit *rangeValue = dynamic_cast<QLineEdit *>(_dynamicFieldWidget)) {
                QString str = QString("%1%2")
                                  .arg(notification->singleValue().toDouble(), 5, 'f', 2, '0')
...
void NotifyPluginOptionsPage::getOptionsPageValues(NotificationItem *notification)
{
    ...
    notification->setCondition(
        NotifyPluginOptionsPage::conditionValues.indexOf(_dynamicFieldCondition->currentText()));
...
void NotifyPluginOptionsPage::updateConfigView(NotificationItem *notification)
{
    ...
    disconnect(_optionsPage->UAVObject,
               QOverload<const QString &>::of(&QComboBox::currentIndexChanged), this,
               &NotifyPluginOptionsPage::on_changedIndex_UAVObject);
    disconnect(_optionsPage->UAVObjectField,
               QOverload<const QString &>::of(&QComboBox::currentIndexChanged), this,
               &NotifyPluginOptionsPage::on_changedIndex_UAVField);
    ...
    if (-1 != _optionsPage->SoundCollectionList->findText(notification->getCurrentLanguage())) {
        _optionsPage->SoundCollectionList->setCurrentIndex(
            _optionsPage->SoundCollectionList->findText(notification->getCurrentLanguage()));
    } else {
        _optionsPage->SoundCollectionList->setCurrentIndex(
            _optionsPage->SoundCollectionList->findText("default"));
    }
    ...
    if (-1 != _optionsPage->UAVObject->findText(notification->getDataObject())) {
        _optionsPage->UAVObject->setCurrentIndex(
            _optionsPage->UAVObject->findText(notification->getDataObject()));
    }
    ...
    if (-1 != _optionsPage->UAVObjectField->findText(notification->getObjectField())) {
        _optionsPage->UAVObjectField->setCurrentIndex(
            _optionsPage->UAVObjectField->findText(notification->getObjectField()));
    }

    if (-1 != _optionsPage->Sound1->findText(notification->getSound1())) {
        _optionsPage->Sound1->setCurrentIndex(
            _optionsPage->Sound1->findText(notification->getSound1()));
    } else {
    ...
    if (-1 != _optionsPage->Sound2->findText(notification->getSound2())) {
        _optionsPage->Sound2->setCurrentIndex(
            _optionsPage->Sound2->findText(notification->getSound2()));
    } else {
        // show item from default location
        _optionsPage->SoundCollectionList->setCurrentIndex(
            _optionsPage->SoundCollectionList->findText("default"));
    ...
    if (-1 != _optionsPage->Sound3->findText(notification->getSound3())) {
        _optionsPage->Sound3->setCurrentIndex(
            _optionsPage->Sound3->findText(notification->getSound3()));
    } else {
        // show item from default location
        _optionsPage->SoundCollectionList->setCurrentIndex(
            _optionsPage->SoundCollectionList->findText("default"));
...
void NotifyPluginOptionsPage::on_changedSelection_notifyTable(
...
void NotifyPluginOptionsPage::on_clicked_buttonAddNotification()
{
    ...
    if (((!_optionsPage->Sound2->currentText().isEmpty())
         && (_sayOrder->currentText() == "After second"))
        || ((!_optionsPage->Sound3->currentText().isEmpty())
            && (_sayOrder->currentText() == "After third"))) {
        delete notification;
        return;
    } else {
        notification->setSayOrder(_sayOrder->currentIndex());
    ...
    _notifyRulesSelection->setCurrentIndex(
        _notifyRulesModel->index(_privListNotifications.size() - 1, 0, QModelIndex()),
...
void NotifyPluginOptionsPage::on_clicked_buttonModifyNotification()
{
    NotificationItem *notification = new NotificationItem;
    getOptionsPageValues(notification);
    notification->setRetryValue(
        _privListNotifications.at(_notifyRulesSelection->currentIndex().row())->retryValue());
    notification->setLifetime(
        _privListNotifications.at(_notifyRulesSelection->currentIndex().row())->lifetime());
    notification->setMute(
        _privListNotifications.at(_notifyRulesSelection->currentIndex().row())->mute());
