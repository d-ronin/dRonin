...
// constructor
OPMapGadgetWidget::OPMapGadgetWidget(QWidget *parent)
    : QFrame(parent)
{
    // **************
    ...
    m_maxUpdateRate =
        max_update_rate_list[4]; // 2 seconds //SHOULDN'T THIS BE LOADED FROM THE USER PREFERENCES?
    ...
    m_map->SetMouseWheelZoomType(
        internals::MouseWheelZoomType::MousePositionWithoutCenter); // set how the mouse wheel zoom
                                                                    // functions
    ...
    m_map->Home->SetSafeArea(
        safe_area_radius_list[0]); // set radius (meters) //SHOULDN'T THE DEFAULT BE USER DEFINED?
    m_map->Home->SetShowSafeArea(
        true); // show the safe area  //SHOULDN'T THE DEFAULT BE USER DEFINED?
    m_map->Home->SetToggleRefresh(true);

    if (m_map->Home)
        connect(m_map->Home, &mapcontrol::HomeItem::homedoubleclick, this,
                &OPMapGadgetWidget::onHomeDoubleClick);
    ...
    connect(m_map, &mapcontrol::TLMapWidget::zoomChanged, this,
            &OPMapGadgetWidget::zoomChanged); // map zoom change signals
    connect(m_map, &mapcontrol::TLMapWidget::OnCurrentPositionChanged, this,
            &OPMapGadgetWidget::OnCurrentPositionChanged); // map poisition change signals
    connect(m_map, &mapcontrol::TLMapWidget::OnTileLoadComplete, this,
            &OPMapGadgetWidget::OnTileLoadComplete); // tile loading stop signals
    connect(m_map, &mapcontrol::TLMapWidget::OnTileLoadStart, this,
            &OPMapGadgetWidget::OnTileLoadStart); // tile loading start signals
    connect(m_map, &mapcontrol::TLMapWidget::OnTilesStillToLoad, this,
            &OPMapGadgetWidget::OnTilesStillToLoad); // tile loading signals
    connect(m_map, &mapcontrol::TLMapWidget::OnWayPointDoubleClicked, this,
            &OPMapGadgetWidget::wpDoubleClickEvent);
    ...
    if (pm) {
        ...
        if (uavo_mgr) {
            UAVDataObject *obj =
                dynamic_cast<UAVDataObject *>(uavo_mgr->getObject(QString("HomeLocation")));
            if (obj) {
                connect(obj, &UAVObject::objectUpdated, this,
                        &OPMapGadgetWidget::homePositionUpdated);
            }
        }
        ...
        if (telMngr) {
            connect(telMngr, &TelemetryManager::connected, this,
                    &OPMapGadgetWidget::onTelemetryConnect);
            connect(telMngr, &TelemetryManager::disconnected, this,
                    &OPMapGadgetWidget::onTelemetryDisconnect);
        }
    }
    ...
    connect(windVelocityActual, &UAVObject::objectUpdated, this,
            &OPMapGadgetWidget::updateWindspeed);
...
{ // the user has right clicked on the map - create the pop-up context menu and display it
    ...
    s = QString::number(m_context_menu_lat_lon.Lat(), 'f', 7) + "  "
        + QString::number(m_context_menu_lat_lon.Lng(), 'f', 7);
    ...
    QMenu maxUpdateRateSubMenu(
        tr("&Max Update Rate ") + "(" + QString::number(m_maxUpdateRate) + " ms)", this);
    ...
    QMenu safeAreaSubMenu(
        tr("Safe Area Radius") + " (" + QString::number(m_map->Home->SafeArea()) + "m)", this);
    ...
    QMenu uavTrailTypeSubMenu(
        tr("UAV trail type") + " ("
            + mapcontrol::Helper::StrFromUAVTrailType(m_map->UAV->GetTrailType()) + ")",
        this);
    ...
    QMenu uavTrailTimeSubMenu(
        tr("UAV trail time") + " (" + QString::number(m_map->UAV->TrailTime()) + " sec)", this);
    ...
    QMenu uavTrailDistanceSubMenu(tr("UAV trail distance") + " ("
                                      + QString::number(m_map->UAV->TrailDistance()) + " meters)",
                                  this);
...
void OPMapGadgetWidget::updatePosition()
{
    ...
    // Convert angular velocities into a rotationg rate around the world-frame yaw axis. This is
    // found by simply taking the dot product of the angular Euler-rate matrix with the angular
    // rates.
    float psiRate_dps = 0 * gyrosData.z
        + sin(attitudeActualData.Roll * deg_to_rad) / cos(attitudeActualData.Pitch * deg_to_rad)
            * gyrosData.y
        + cos(attitudeActualData.Roll * deg_to_rad) / cos(attitudeActualData.Pitch * deg_to_rad)
            * gyrosData.z;
    ...
    QString str = "lat: " + QString::number(uav_pos.Lat(), 'f', 7) + " lon: "
        + QString::number(uav_pos.Lng(), 'f', 7) + " " + QString::number(uav_yaw, 'f', 1) + "deg"
...
void OPMapGadgetWidget::updateMousePos()
{
    ...
    internals::PointLatLng lat_lon =
        m_map->GetFromLocalToLatLng(p); // fetch the current lat/lon mouse position
    ...
    QString s1 = QString::number(m_mouse_lat_lon.Lat(), 'f', 7) + "  "
        + QString::number(m_mouse_lat_lon.Lng(), 'f', 7);
    if (wp) {
    ...
    Utils::CoordinateConversions().LLA2NED_HomeLLA(LLA, homeLLA, NED); //<-- This yields, NED, the
                                                                       //tile size in meters,
                                                                       //assuming we are at the
                                                                       //origin in a plate carre
                                                                       //system
...
void OPMapGadgetWidget::zoomChanged(double zoomt, double zoom, double zoomd)
{
    ...
    QString s = "tot:" + QString::number(zoomt, 'f', 1) + " rea:" + QString::number(zoom, 'f', 1)
        + " dig:" + QString::number(zoomd, 'f', 1);
...
void OPMapGadgetWidget::OnCurrentPositionChanged(internals::PointLatLng point)
{
    ...
    QString coord_str =
        QString::number(point.Lat(), 'f', 7) + "   " + QString::number(point.Lng(), 'f', 7) + " ";
...
void OPMapGadgetWidget::OnTilesStillToLoad(int number)
{
    ...
    m_widget->progressBarMap->setValue(m_widget->progressBarMap->maximum()
                                       - number); // update the progress bar
...
void OPMapGadgetWidget::createActions()
{
    ...
    copyMouseLatLonToClipAct->setStatusTip(
        tr("Copy the mouse latitude and longitude to the clipboard"));
    connect(copyMouseLatLonToClipAct, &QAction::triggered, this,
            &OPMapGadgetWidget::onCopyMouseLatLonToClipAct_triggered);
    ...
    connect(copyMouseLatToClipAct, &QAction::triggered, this,
            &OPMapGadgetWidget::onCopyMouseLatToClipAct_triggered);
    ...
    connect(copyMouseLonToClipAct, &QAction::triggered, this,
            &OPMapGadgetWidget::onCopyMouseLonToClipAct_triggered);
    ...
    connect(showCompassRoseAct, &QAction::toggled, this,
            &OPMapGadgetWidget::onShowCompassRoseAct_toggled);
    ...
    connect(showWindCompassAction, &QAction::toggled, this,
            &OPMapGadgetWidget::onShowWindCompassAction_toggled);
    ...
    connect(showDiagnostics, &QAction::toggled, this,
            &OPMapGadgetWidget::onShowDiagnostics_toggled);
    ...
    changeDefaultLocalAndZoom->setStatusTip(
        tr("Changes the map default zoom and location to the current values"));
    connect(changeDefaultLocalAndZoom, &QAction::triggered, this,
            &OPMapGadgetWidget::onChangeDefaultLocalAndZoom);
    ...
    connect(goMouseClickAct, &QAction::triggered, this,
            &OPMapGadgetWidget::onGoMouseClickAct_triggered);
    ...
    followUAVpositionAct->setChecked(false);
    connect(followUAVpositionAct, &QAction::toggled, this,
            &OPMapGadgetWidget::onFollowUAVpositionAct_toggled);
    ...
    connect(followUAVheadingAct, &QAction::toggled, this,
            &OPMapGadgetWidget::onFollowUAVheadingAct_toggled);
    ...
    connect(wayPointEditorAct, &QAction::triggered, this,
            &OPMapGadgetWidget::onOpenWayPointEditorAct_triggered);
    ...
    connect(addWayPointActFromContextMenu, &QAction::triggered, this,
            &OPMapGadgetWidget::onAddWayPointAct_triggeredFromContextMenu);
    ...
    connect(addWayPointActFromThis, &QAction::triggered, this,
            &OPMapGadgetWidget::onAddWayPointAct_triggeredFromThis);
    ...
    connect(editWayPointAct, &QAction::triggered, this,
            &OPMapGadgetWidget::onEditWayPointAct_triggered);
    ...
    connect(lockWayPointAct, &QAction::triggered, this,
            &OPMapGadgetWidget::onLockWayPointAct_triggered);
    ...
    connect(deleteWayPointAct, &QAction::triggered, this,
            &OPMapGadgetWidget::onDeleteWayPointAct_triggered);
    ...
    connect(clearWayPointsAct, &QAction::triggered, this,
            &OPMapGadgetWidget::onClearWayPointsAct_triggered);

    overlayOpacityActGroup = new QActionGroup(this);
    connect(overlayOpacityActGroup, &QActionGroup::triggered, this,
            &OPMapGadgetWidget::onOverlayOpacityActGroup_triggered);
    ...
    connect(homeMagicWaypointAct, &QAction::triggered, this,
            &OPMapGadgetWidget::onHomeMagicWaypointAct_triggered);

    mapModeActGroup = new QActionGroup(this);
    connect(mapModeActGroup, &QActionGroup::triggered, this,
            &OPMapGadgetWidget::onMapModeActGroup_triggered);
    mapModeAct.clear();
    {
    ...
    connect(zoomActGroup, &QActionGroup::triggered, this,
            &OPMapGadgetWidget::onZoomActGroup_triggered);
    zoomAct.clear();
    ...
    connect(maxUpdateRateActGroup, &QActionGroup::triggered, this,
            &OPMapGadgetWidget::onMaxUpdateRateActGroup_triggered);
    ...
    connect(showSafeAreaAct, &QAction::toggled, this,
            &OPMapGadgetWidget::onShowSafeAreaAct_toggled);

    safeAreaActGroup = new QActionGroup(this);
    connect(safeAreaActGroup, &QActionGroup::triggered, this,
            &OPMapGadgetWidget::onSafeAreaActGroup_triggered);
    safeAreaAct.clear();
    ...
    connect(uavTrailTypeActGroup, &QActionGroup::triggered, this,
            &OPMapGadgetWidget::onUAVTrailTypeActGroup_triggered);
    ...
    for (int i = 0; i < uav_trail_type_list.count(); i++) {
        mapcontrol::UAVTrailType::Types uav_trail_type =
            mapcontrol::Helper::UAVTrailTypeFromString(uav_trail_type_list[i]);
        QAction *uavTrailType_act = new QAction(
            mapcontrol::Helper::StrFromUAVTrailType(uav_trail_type), uavTrailTypeActGroup);
    ...
    connect(showTrailLineAct, &QAction::toggled, this,
            &OPMapGadgetWidget::onShowTrailLineAct_toggled);

    clearUAVtrailAct = new QAction(tr("Clear UAV trail"), this);
    clearUAVtrailAct->setStatusTip(tr("Clear the UAV trail"));
    connect(clearUAVtrailAct, &QAction::triggered, this,
            &OPMapGadgetWidget::onClearUAVtrailAct_triggered);

    uavTrailTimeActGroup = new QActionGroup(this);
    connect(uavTrailTimeActGroup, &QActionGroup::triggered, this,
            &OPMapGadgetWidget::onUAVTrailTimeActGroup_triggered);
    ...
    for (int i = 0; i < list_size; i++) {
        ...
        QAction *uavTrailTime_act =
            new QAction(QString::number(uav_trail_time) + " sec", uavTrailTimeActGroup);
    ...
    connect(uavTrailDistanceActGroup, &QActionGroup::triggered, this,
            &OPMapGadgetWidget::onUAVTrailDistanceActGroup_triggered);
    ...
    for (int i = 0; i < list_size; i++) {
        ...
        QAction *uavTrailDistance_act =
            new QAction(QString::number(uav_trail_distance) + " meters", uavTrailDistanceActGroup);
...
void OPMapGadgetWidget::onCopyMouseLatLonToClipAct_triggered()
{
    ...
    clipboard->setText(QString::number(m_context_menu_lat_lon.Lat(), 'f', 7) + ", "
                           + QString::number(m_context_menu_lat_lon.Lng(), 'f', 7),
...
{
    QClipboard *clipboard = QApplication::clipboard();
    clipboard->setText(QString::number(m_context_menu_lat_lon.Lng(), 'f', 7),
                       QClipboard::Clipboard);
}
...
{
    emit defaultLocationAndZoomChanged(m_map->CurrentPosition().Lng(),
                                       m_map->CurrentPosition().Lat(), m_map->ZoomTotal());
}
...
void OPMapGadgetWidget::onGoMouseClickAct_triggered()
{
    ...
    m_map->SetCurrentPosition(
        m_map->currentMousePosition()); // center the map onto the mouse position
}
...
void OPMapGadgetWidget::onSetHomeAct_triggered()
{
    ...
    altitude = QInputDialog::getDouble(this, tr("Set home altitude"),
                                       tr("In [m], referenced to WGS84 geoid:"), altitude, -100,
                                       100000, 2, &ok);
...
void OPMapGadgetWidget::onGoUAVAct_triggered()
{
    ...
    {
        internals::PointLatLng uav_pos =
            internals::PointLatLng(latitude, longitude); // current UAV position
...
void OPMapGadgetWidget::onUAVTrailTypeActGroup_triggered(QAction *action)
{
    ...
    QStringList uav_trail_type_list = mapcontrol::Helper::UAVTrailTypes();
    mapcontrol::UAVTrailType::Types uav_trail_type =
        mapcontrol::Helper::UAVTrailTypeFromString(uav_trail_type_list[trail_type_idx]);
...
// *************************************************************************************
// return a destination lat/lon point given a source lat/lon point and the bearing and distance from
// the source point

internals::PointLatLng OPMapGadgetWidget::destPoint(internals::PointLatLng source, double bear,
                                                    double dist)
{
...
void OPMapGadgetWidget::setMapFollowingMode()
{
    ...
    if (!followUAVpositionAct->isChecked()) {
        ...
        m_map->UAV->SetMapFollowType(UAVMapFollowType::CenterMap); // the map library won't let you
                                                                   // reset the uav rotation if it's
                                                                   // already in rotate mode
...
bool OPMapGadgetWidget::setHomeLocationObject()
{
    ...
    double LLA[3] = { m_home_position.coord.Lat(), m_home_position.coord.Lng(),
                      m_home_position.altitude };
...
void OPMapGadgetWidget::on_tbFind_clicked()
{
    ...
    GeoCoderStatusCode::Types result =
        m_map->SetCurrentPositionByKeywords(m_widget->leFind->text());
...
void OPMapGadgetWidget::updateWindspeed(UAVObject *obj)
{
    ...
    double windVelocity_NED[3] = { windVelocityActualData.North, windVelocityActualData.East,
                                   windVelocityActualData.Down };
