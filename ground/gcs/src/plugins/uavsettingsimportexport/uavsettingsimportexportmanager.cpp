    ...
    : QObject(parent)
{
    ...
    cmd = am->registerAction(new QAction(this), "UAVSettingsImportExportPlugin.UAVSettingsImport",
                             QList<int>() << Core::Constants::C_GLOBAL_ID);
    ...
    cmd = am->registerAction(new QAction(this), "UAVSettingsImportExportPlugin.UAVDataExport",
                             QList<int>() << Core::Constants::C_GLOBAL_ID);
...
void UAVSettingsImportExportManager::extensionsInitialized()
{
    ...
    connect(telemetry, &TelemetryManager::connectedChanged, this,
            &UAVSettingsImportExportManager::setCommandsEnabled);
...
bool UAVSettingsImportExportManager::importUAVSettings(const QByteArray &settings, bool quiet)
{
        ...
        if (e.tagName() == "object") {
            ...
            if (obj == NULL) {
                        ...
                        if (uavfield) {
                            ...
                            if (list.length() == 1) {
                                if (false == uavfield->checkValue(f.attribute("values"))) {
                                    qDebug() << "checkValue returned false on: " << uavObjectName
                                             << f.attribute("values");
                                    setError = true;
                                } else {
                                ...
                                foreach (QString element, list) {
                                    if (false == uavfield->checkValue(element, i)) {
                                        qDebug() << "checkValue(list) returned false on: "
                                                 << uavObjectName << list;
                                        setError = true;
                                    } else {
                ...
                if (error) {
                    ...
                    qDebug() << "Mismatch for Object " << uavObjectName << uavObjectID << " - "
                             << newObj->getObjID();
    ...
    if (swui.numLines() < 1) {
        QMessageBox::critical(dynamic_cast<QWidget *>(Core::ICore::instance()->mainWindow()),
                              tr("Unable to import settings"),
                              tr("No settings found in XML dump; the flight controller settings "
                                 "may have been blank."),
                              QMessageBox::Ok);
...
void UAVSettingsImportExportManager::importUAVSettings()
{
    ...
    fileName =
        QFileDialog::getOpenFileName(dynamic_cast<QWidget *>(Core::ICore::instance()->mainWindow()),
...
QString UAVSettingsImportExportManager::createXMLDocument(const enum storedData what,
    ...
    foreach (QVector<UAVDataObject *> list, objList) {
        foreach (UAVDataObject *obj, list) {
            if (!obj->getIsPresentOnHardware())
                continue;
            if (((what == Settings) && obj->isSettings()) || ((what == Data) && !obj->isSettings())
                || (what == Both)) {
                ...
                o.setAttribute("id",
                               QString("0x") + QString().setNum(obj->getObjID(), 16).toUpper());
                if (fullExport) {
                    QDomElement d = doc.createElement("description");
                    QDomText t = doc.createTextNode(
                        obj->getDescription().remove("@Ref ", Qt::CaseInsensitive));
                    d.appendChild(t);
                    o.appendChild(d);
                }
...
void UAVSettingsImportExportManager::exportUAVSettings()
{
    ...
    fileName =
        QFileDialog::getSaveFileName(dynamic_cast<QWidget *>(Core::ICore::instance()->mainWindow()),
    ...
    if (file.open(QIODevice::WriteOnly) && (file.write(xml.toLatin1()) != -1)) {
        ...
        QMessageBox::critical(dynamic_cast<QWidget *>(Core::ICore::instance()->mainWindow()),
                              tr("UAV Settings Export"), tr("Unable to save settings: ") + fileName,
                              QMessageBox::Ok);
        return;
    }
