...
void ImportSummaryDialog::openHelp()
{
    QDesktopServices::openUrl(
        QUrl("https://github.com/d-ronin/dRonin/wiki/OnlineHelp:-UAV-Settings-import-export",
             QUrl::StrictMode));
}
...
void ImportSummaryDialog::doTheApplySaving()
{
    ...
    if (!quiet) {
        ...
        msgBox.setInformativeText(tr("You must power cycle the flight controller to apply settings "
...
void ImportSummaryDialog::showEvent(QShowEvent *event)
{
    ...
    int width = ui->importSummaryList->width()
        - (ui->importSummaryList->columnWidth(0) + ui->importSummaryList->columnWidth(2));
