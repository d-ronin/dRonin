...
/**
 * @brief IBoardType::queryChannelBanks Converts board channel banks into a list of formatted
 * strings
            ...
            if (i == 0) { // First channel in bank
                ...
                if (channelBank.size() > 1) { // If more than 1 channel in bank, add separator
                    if ((channelBank[i] + 1) == channelBank[i + 1]) {
                        banksString.append(
                            QString("-")); // Next channel is sequential, add '-' to the string
                        sequential = true;
                    } else {
                        banksString.append(
                            QString(",")); // Next channel is not sequential, add ',' to the string
                        sequential = false;
                    }
                }
                ...
                if (sequential) {
                    if ((channelBank[i] + 1) == channelBank[i + 1]) { // Still sequential channels,
                                                                      // nothing to add to string
                        sequential = true;
                    } else {
                        banksString.append(QString("%1,").arg(
                            channelBank[i])); // End of sequence, add channel and ',' to string
                        sequential = false;
                    }
                } else {
                    banksString.append(QString("%1").arg(channelBank[i]));
