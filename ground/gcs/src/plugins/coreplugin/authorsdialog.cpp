    ...
    : QDialog(parent)
{
    ...
    QLabel *mainLabel = new QLabel(
        tr("%0 proudly brought to you by this fine team:").arg(GCS_PROJECT_BRANDING_PRETTY));
    ...
    relCreditsArea->setSource(
        QUrl::fromLocalFile(Utils::PathUtils::InsertDataPath("%%DATAPATH%%gcsrelauthors.html")));

    QLabel *creditsLabel =
        new QLabel(tr("All time (including previous contributions under OpenPilot and Tau Labs):"));
    creditsLabel->setWordWrap(true);
    QTextBrowser *creditsArea = new QTextBrowser(this);
    creditsArea->setSource(
        QUrl::fromLocalFile(Utils::PathUtils::InsertDataPath("%%DATAPATH%%gcsauthors.html")));
