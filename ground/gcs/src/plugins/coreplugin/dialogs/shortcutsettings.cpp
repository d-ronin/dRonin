...
QWidget *ShortcutSettings::createPage(QWidget *parent)
{
    ...
    connect(m_page->resetButton, SIGNAL(clicked()), this, SLOT(resetKeySequence()));
    connect(m_page->removeButton, SIGNAL(clicked()), this, SLOT(removeKeySequence()));
    connect(m_page->defaultButton, SIGNAL(clicked()), this, SLOT(defaultAction()));
...
void ShortcutSettings::initialize()
{
    ...
    foreach (Command *c, m_am->commands()) {
        ...
        if (c->action()) {
            QString text = c->hasAttribute(Command::CA_UpdateText) && !c->defaultText().isNull()
                ? c->defaultText()
                : c->action()->text();
            text.remove(QRegExp("&(?!&)"));
...
int ShortcutSettings::translateModifiers(Qt::KeyboardModifiers state, const QString &text)
{
    int result = 0;
